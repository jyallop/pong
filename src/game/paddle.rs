...
pub fn computer_movement(time: Res<Time>,
	...
	if let Ok((_computer, mut transform)) = queries.q0_mut().single_mut() {
	    ...
	    println!("Difference: {:?}, window_height: {:?}", paddle_location.y - ball_center.y, window.height());
	    if (paddle_location.y - ball_center.y).abs() > (window.height() / 1000.0) {
		println!("passed test");
